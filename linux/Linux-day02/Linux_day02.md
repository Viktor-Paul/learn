...
# 13、本地文件复制到远程服务器
  ...
  eg: scp python.tar.gz tarena@172.40.74.145:/home/tarena/
```
...
# 或者是grep 'tarena' passwd
6、查看passwd文件的权限,并将其权限修改为所有用户都可读可写但是不可执行(chmod命令)
# ls -l passwd
# chmod 666 passwd
7、将 MrRight 目录打包压缩,MrRight-你的名字.tar.gz
# tar -zcvf MrRight.tar.gz MrRight/
8、将此压缩包远程复制到 主讲机 | 同桌 计算机的电脑上
# scp MrRigth.tar.gz tarena@172.40.91.200:/home/tarena/homework/
```
...
1、每小时的第3分钟和第15分钟执行
 # 3,15 * * * *
2、每周六、周日的0点执行一个 backup.py 文件
# 0 0 * * 0,6 python3 backup.py
3、每天18:00到23:00之间每小时执行 backup.py 文件
# 0 18-23/1 * * *  python3 backup.py
```
...
```python
练习：
使用Linux正则匹配mac地址 40:8d:5c:f2:29:aa
cat ip.txt | grep 'ether' | awk '{print $2}'
# 提取本机的mac地址
...
2.执行shell脚本的方法
	1.bash firsh.sh
  2.sh firsh.sh
  3.chmod +x xxx.sh 加x权限，执行./xxx.sh
   然后执行 ./firsh.sh
# t1.sh,要求在主目录下创建目录Aid1904/mydir1/A
  mkdir -p /home/tarena/AID1904/mydier1/A
  tarena@tarena:~$ chmod +x t1.sh
  tarena@tarena:~$ ./t1.sh
  ok
  tarena@tarena:~$ 
```
