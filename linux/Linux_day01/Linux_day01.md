...
'''
tarena@tarena:~$ sudo vi /etc/passwd
[sudo] tarena 的密码： 
tarena@tarena:~$ head -5 /etc/passwd
...
tarena@tarena:~$ ^C
tarena@tarena:~$ cat /etc/passwd | head -5
root:x:0:0:root:/root:/bin/bash
daemon:x:1:1:daemon:/usr/sbin:/usr/sbin/nologin
bin:x:2:2:bin:/bin:/usr/sbin/nologin
sys:x:3:3:sys:/dev:/usr/sbin/nologin
sync:x:4:65534:sync:/bin:/bin/sync
tarena@tarena:~$ cat /etc/passwd | head -5 | tail -2
sys:x:3:3:sys:/dev:/usr/sbin/nologin
sync:x:4:65534:sync:/bin:/bin/sync
'''
ps -aux | grep 'mysql' 筛选行
...
9、chmod 权限 file
  给文件指定或者增加某权限
   chmod 666 file
  chmod a+x file(all)
  chmod g+x file(group)
  chmod u+x file(user)
  chmod o+x file(other)
ls -l file 查看权限
  三组权限     谁创建的 所属组  文件大小
drwxrwxr-x 2 tarena tarena   24576 8月  20 17:16 full
drwxr-xr-x 2 tarena tarena   12288 8月  20 17:16 图片
-rw-r--r-- 1 tarena tarena 9542375 8月  20 17:17 图片.zip
整个文件 - 代表文件 
r-4权限：
w-2权限：
x-1权限：可执行权限
第一组权限rw-:文件的所有者
第二组权限r--:同组的其他用户(tarena2)
第三组权限r--:其他组的其他用户(mysql)
