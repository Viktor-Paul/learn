...
>5.浏览器接收到响应内容，解析展示
前端知识：
<!-- URL域名：统一资源定位符，例如：网址www.baidu.com，当我们在地址栏输入网址时，组织为一个请求（http:）
然后使用tcp发送，发送给后端服务器，我们称之为http请求

通过浏览器软件来帮助我们获取资源，
通过域名解析
webserver tcp解析（按照请求格式），然后给后端网站的应用，然后进行逻辑处理，计算处理，文件处理，数据库处理
假如处理好之后，发送给webserver,然后组织数据，通过tcp发送（客户端浏览器），我们称这个过程为http响应
浏览器进行解析数据，呈现数据
新浪，搜狐 -->
...
<!-- * 请求头：对请求的进一步解释和描述，由键值对构成，是有http协议规定的 -->
```
