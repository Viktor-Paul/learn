...
ZRANK key member  # 0 倒数第一
ZREVRANK key member # 0 正数第一
...
问题：如何获取三款手机的销量排名？
ZUNIONSTORE mobile-001:003 3 mobile-001 mobile-002 mobile-003 # 可否？ 不可以，因为默认聚合为SUM

# 正确
1、ZUNIONSTORE mobile-001:003 mobile-001 mobile-002 mobile-003 AGGREGATE MAX
2、ZREVRANGE mobile-001:003 0 -1 withscores
```
...
# 并集参数: (key,(z1,z2,z3),aggregate='max')
...
# 两个命令比较
SAVE比BGSAVE快，因为BGSAVE需要创建子进程，消耗额外的内存
...
- **数据持久化分类之 - AOF（AppendOnlyFile，默认未开启-存命令）**
...
1、在数据库中创建库 blog_server，指定字符编码utf8（==如已经做过则不用再做==）
...
2、同步数据库，并在user_profile中插入表记录（==如已经做过则不用再做==）
