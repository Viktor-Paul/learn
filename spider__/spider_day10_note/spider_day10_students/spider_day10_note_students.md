...
- **使用redis_key改写**
  ...
  ```python
  前提：Scrapy项目爬虫文件是基于start_urls结构写的
  # 第一步: settings.py无须改动
...
然后把 bind-address = 127.0.0.1 注释掉
2.重启MySQL
sudo /etc/init.d/mysql restart
3.用户授权
mysql -uroot -p123456
mysql>grant all privileges on *.* //库名.表名 to "用户名"@"%" identified by '密码' with grant option;
...
def process_request(self, request, spider):
        # 先把cookies处理为字典
        cookies = {}
        string = "DICT_UGC=be3af0da19b5c5e6aa4e17bd8d90b28a|; OUTFOX_SEARCH_USER_ID=873357026@111.201.229.83; JSESSIONID=abcdhSO_khoUJ7e6X0kYw; OUTFOX_SEARCH_USER_ID_NCOO=236931893.40747365; ___rl__test__cookies=1565698477681"
        cookie_list = string.split(';')
        for key in cookie_list:
            cookies[key.split('=')[0]] = key.split('=')[1]
