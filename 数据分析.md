...
```python
dates, bhp_closing_prices = np.loadtxt( '../data/bhp.csv', delimiter=',', 
                                       usecols=(1,6), dtype='M8[D], f8',
                                       converters={1:dmy2ymd}, unpack=True)
vale_closing_prices = np.loadtxt( '../data/vale.csv', delimiter=',', 
                                 usecols=(6), dtype='f8',converters={1:dmy2ymd}, 
                                 unpack=True)
...
#绘制收益线
mp.plot(dates, bhp_returns, color='dodgerblue', linestyle='--', 
        label='bhp_returns', alpha=0.3)
mp.plot(dates, vale_returns, color='orangered', linestyle='--',
        label='vale_returns', alpha=0.3)
#绘制卷积降噪线
mp.plot(dates[7:], bhp_returns_convolved, color='dodgerblue', 
        label='bhp_returns_convolved', alpha=0.5)
...
print(a.cumprod())
------------------------------------------
[ 2  4  6  8 10 12 14 16 18] <class 'numpy.ndarray'>
45 <class 'numpy.int64'>
[ 1  3  6 10 15 21 28 36 45] <class 'numpy.ndarray'>
[[11 12 13 14 15 16 17 18 19]
 [21 22 23 24 25 26 27 28 29]
 [31 32 33 34 35 36 37 38 39]] <class 'numpy.ndarray'>
...
```python
# 产生size个随机数，每个随机数t为在总样本中随机抽取nsample个样本后好样本的个数，
...
```python
indices = numpy.lexsort((second column, First column))
```
...
scipy提供了常见的插值算法可以通过一组离散点得到符合这组散点分布规律 的插值器函数。若我们给插值器函数更多的散点x坐标序列，该函数将会返回相应的y坐标序列。
